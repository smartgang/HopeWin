...
'''
'''
from MacdBiasWin import MacdBiasWin
import pandas as pd
import numpy as np
import os
...
def getResult(strategyName, symbolinfo, K_MIN, setname, rawdataDic, para, result_para_dic, indexcols, timestart):
    ...
    for symbol in symbollist:
        ...
        r = MacdBiasWin(symbolinfo=symbolinfo, rawdata_macd=rawdata, macdParaSet=para)
...
def getParallelResult(strategyParameter, resultpath, parasetlist, paranum, indexcols):
    ...
    for i in range(0, paranum):
        ...
        bias_n = parasetlist.ix[i, 'BIAS_N']
        bias_up = parasetlist.ix[i, 'BIAS_UP']
        bias_down = parasetlist.ix[i, 'BIAS_DOWN']
        paraset = {
            'Setname': setname,
            'MACD_S': macd_s,
            'MACD_L': macd_l,
